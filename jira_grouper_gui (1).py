...
def build_slack_blocks(issues, domain, mentions):
    """Block Kit rich_text 블록 생성 — 불렛 리스트 정상 렌더링"""
    groups = {}
    for iss in issues:
        a = (iss["fields"].get("assignee") or {}).get("displayName", "미배정")
        groups.setdefault(a, []).append(iss)
...
def send_slack_dm(bot_token, user_id, blocks, fallback_text):
    """Slack API chat.postMessage — Block Kit rich_text로 DM 전송"""
    import urllib.request, urllib.error
    url = "https://slack.com/api/chat.postMessage"
    payload = json.dumps({
        "channel": user_id,
        "text": fallback_text,   # 알림 미리보기용
        "blocks": blocks,
    }).encode("utf-8")
...
class App(tk.Tk):
    ...
    def _send_slack(self):
        ...
        domain   = self.domain_var.get().strip()
        mentions = self.cfg.get("slack_mentions", {})
        blocks   = build_slack_blocks(self._last_issues, domain, mentions)
        fallback = self.result_text.get("1.0", "end").strip()

        self.send_btn.configure(state="disabled", text="전송 중…")

        def worker():
            try:
                send_slack_dm(bot_token, my_uid, blocks, fallback)
